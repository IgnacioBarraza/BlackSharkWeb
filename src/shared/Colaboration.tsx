...
export const Colaboration = () => {
  ...
  const { deleteImageFromCollaboration, uploadCollaborationImage } =
    useFirebase()
  ...
  const [selectedCollab, setSelectedCollab] = useState<Colaborations | null>(
    null
  )
  ...
  const handleImageUpload = () => {
    if (selectedServices.length === 0)
      return alert(
        'Debes seleccionar al menos un servicio asociado a la colaboración!'
      )
    ...
    if (confirm('¿Quieres eliminar esta colaboración?')) {
      ...
      try {
        ...
        if (status === 200) {
          ...
          const updatedCollabs = colaboration.filter(
            (colab) => colab.id_collaboration !== item.id_collaboration
          )
      ...
      <div className='flex justify-center bg-gray-100 fit w-full'>
        <div className=' grid grid-cols-2 gap-20 py-10'>
          {userType === 'admin' && userToken && (
            <button
              onClick={handleModal}
              className='w-[100px] md:w-[300px] h-64 flex items-center justify-center rounded-lg shadow-md border hover:bg-gray-200 border-gray-300 transition duration-300 ease-in-out transform hover:scale-105'
            >
